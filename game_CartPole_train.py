...
class Net:
    ...
    def __init__(self):
        self.observation = tf.placeholder(dtype=tf.float32, shape=[None, 4])  #nv
    ...
    def forward(self, discount):
        ...
        self.pre_q = tf.expand_dims(tf.reduce_sum(tf.multiply(tf.squeeze(tf.one_hot(self.action,2),axis=1),self.pre_qs),axis=1),axis=1)
    ...
    with tf.Session()  as sess:
        ...
        for k in range(10000000):
            ...
            if k>1000:
                ...
                print("**************************************************")
                print("**************************************************")
                for ii in range(1000000000000):
                    run_observation = game.reset()
                    game.render()
                    run_action = sess.run(run_action_op, feed_dict={
                        net.observation: [run_observation]
                    })[0]
                    observation, reward, done, info=game.env.step(run_action)
                    print(ii)
                    if done:
